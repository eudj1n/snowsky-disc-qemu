...
REVISION="$(cat "$REPO/experiments/diskos/source-revision")"
...
(cd "$REPO" && python3 -B -m experiments.diskos.prepare "$BUILD")
# Keep the adapter and its license beside the adapted upstream source.
mkdir -p "$BUILD/emulator-adapter"
cp "$REPO/experiments/diskos/emu_io.c" "$BUILD/emulator-adapter/emu_io.c"
