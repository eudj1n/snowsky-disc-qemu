...
from experiments.disc_assistant.assistant.nlu.intents import Intent, AlbumIntent, ControlIntent, LanguageIntent, VolumeIntent, parse
...
class Interpretation:
    ...
    intent: Intent | AlbumIntent | ControlIntent | LanguageIntent | VolumeIntent | None = None
...
class RuleInterpreter:
    info = ProviderInfo('rules', RULES_VERSION, 'local')
...
def validate_intent(intent):
    ...
    if type(intent) is ControlIntent:
        valid = intent.action in ('pause', 'resume', 'stop', 'next', 'previous', 'like', 'dislike', 'now_playing')
    elif type(intent) is VolumeIntent:
        valid = ((type(intent.value) is int and 0 <= intent.value <= 120 and intent.direction is None)
                 or (intent.value is None and intent.direction in ('up', 'down')))
...
async def _interpret_request(text, context, *, interpreter=None, trace=None):
    ...
    try:
        from experiments.disc_assistant.assistant.nlu.understanding import single_action
...
async def interpret_request(text, context, *, interpreter=None, trace=None, shadow=None, shadow_timeout_ms=100):
    ...
    if not isinstance(text, str) or not 1 <= len(text) <= 1000 or any(ord(c) < 32 for c in text):
        raise ValueError('command must contain 1..1000 characters without control characters')
    from experiments.disc_assistant.assistant.nlu.interpretation_sources import collect, comparison
