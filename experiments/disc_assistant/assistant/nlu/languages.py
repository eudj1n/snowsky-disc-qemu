...
LOCALES = Path(__file__).parents[1] / 'locales'
SECTIONS = {
...
class LanguageRules:
    ...
    def prefix(self, section, text):
        ...
        for phrase, meaning in getattr(self, section):
            if section == 'commands' and meaning not in ('play', 'set_language', 'volume'):
                continue
...
def load_languages(enabled=DEFAULT_LANGUAGES, *, directory=LOCALES):
    ...
    merged = {section: {} for section in (*SECTIONS, 'language_names', 'numbers')}
    for code in enabled:
        ...
        for section, table in raw.items():
            if (not isinstance(table, dict) or (section not in ('language_names', 'numbers') and set(table) - SECTIONS[section])
                    or (section == 'numbers' and any(not re.fullmatch(r'(?:[0-9]|[1-9][0-9]|1[01][0-9]|120)', key) for key in table))
