...
from experiments.disc_assistant.assistant.speech import InvalidSpeech, SpeechUnavailable, Transcription
from experiments.disc_assistant.assistant.voice.process import model_digest
...
class GigaAMServer(SpeechAdapter):
    ...
    def capabilities(self):
        name = Path(self.settings.get('model', '')).stem
        return Capabilities(locales=MODELS.get(name, ()), max_seconds=25)

    def available(self):
        return Path(self.settings.get('model', '')).expanduser().is_file() and bool(self.settings.get('server_url'))
    ...
    def result_evidence(self):
        return {'verified_worker': self.server_evidence} if self.server_evidence else {}
