...
from experiments.disc_assistant.assistant.voice import text as speech_text
...
from experiments.disc_assistant.assistant.speech import SpeechUnavailable, InvalidSpeech
...
class PiperSynthesizer(SpeechAdapter):
    ...
    def capabilities(self):
        return Capabilities(locales=tuple(self.settings.get('models', {})))
    ...
    def evidence(self, locale=None):
        try:
            ...
            return {'model': model.name, 'model_sha256': digest(model), 'config_sha256': digest(config), 'voice': model.stem, 'text_preparation': speech_text.REVISION}
