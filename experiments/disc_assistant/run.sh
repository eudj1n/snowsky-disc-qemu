...
if [[ ${1:-help} == help || ${1:-} == --help || ${1:-} == -h ]]; then
    cat <<'HELP'
Usage: experiments/disc_assistant/run.sh [--config PATH] [--source cli|scheduled] [--language CODE] [--debug] COMMAND [ARGS]
...
cd -- "$repo_dir"
exec "$venv_python" -m experiments.disc_assistant.launcher "$@"
