...
> Use the [current page](../roadmap.md) for active guidance.
...
search. See the [quick guide](../guides/quick-guide.md),
[NLU ownership](../../assistant/nlu/README.md) and
[Python/Controller extraction review](../../../../docs/development/reports/2026-09-21-python-quality.md).


The **software MVP was accepted on 2026-09-19** against the existing 64/64
emulator scenario run. See [acceptance and reproducibility](2026-09-19-mvp-acceptance.md).
Physical acceptance and speech/platform work continue in issues #23/#24.
Start with the [current checkpoint](../status.md) for follow-up work and evidence.
...
resolved. Validation is recorded in the [current checkpoint](../status.md).

Checkpoint: **2026-09-18**. The desktop prototype in
[experiments/disc_assistant](../../README.md) now includes catalog
...
are implemented. [Disc Assistant Web](../guides/web.md) now adds browser microphone
capture and text input through the shared application service. [Piper replies and
common speech installation](../guides/tts.md) are implemented; web uses Whisper
Server. The owner reports microphone play/stop working on a physical player.
Quantified human microphone acceptance and listening history remain pending. See [voice setup and evaluation](../guides/voice.md)
and the [command table](../guides/commands.md).
The [interpreter, speech and locale architecture](../architecture/pipeline.md) is now
implemented. It supersedes the merged-language/separate-response policy recorded
in the historical increments below.
The [command catalog and explanation preview](../reference/command-catalog.md) are
also implemented; learned classification remains diagnostic while the live
interpreter uses rules. The [reviewed-data workflow and v2 model comparison](../evaluation/nlu-data.md) are
implemented. [Independent sources, argument extraction and optional shadow comparison](../architecture/interpretation-sources.md)
...
The current device contract is [DISC capabilities](../../../../docs/protocol/disc-capabilities.md).

The current [MVP boundary and acceptance gate](../reference/mvp.md) is tracked in
...
[Offline shadow reports and review queues](../evaluation/shadow-reports.md) are now
implemented. They support interpretation analysis, not physical-device acceptance.

The [physical baseline](../evaluation/physical-baseline.md) is in progress, with RU-01–19
recorded privately and one disputed expectation kept separate. The owner proposed
[repeatable Assistant scenarios against the emulator](../evaluation/emulator-acceptance.md)
...
verified whole/scoped native playback. See the [command contract](../guides/commands.md#album-selection).
...
See [remote modes](../../../../docs/protocol/remote-modes-themes.md) and [idle power](../../../../emulator/docs/idle-power.md).
...
acceptance justify promotion into the main [repository layout](../../../../docs/architecture/repository.md).
...
See [precedence and migration](../architecture/pipeline.md#migration-and-storage).
...
[request/decision journal](../guides/history.md) while preserving settings;
...
annotations until a match can be established. See [formats](../../../../docs/protocol/formats.md).
...
[architecture contract](../architecture/pipeline.md) and [locale guide](../reference/locales.md).
...
invented selector. Preserve the limitations in [library browsing](../../../../docs/protocol/library-browsing.md).
...
Recorded quality gaps and setup are in [ASSISTANT_VOICE.md](../guides/voice.md).
...
described in [diagnostics](../../../../research/docs/diagnostics.md) and the
...
- M2c implements a persistent connection owned by the foreground Assistant
  ...
  scripts and cron; see [persistent session ownership](../architecture/playback.md#m2c-persistent-device-session).
...
[playback and queue plan](../architecture/playback.md): M2a adds index-independent
...
These rules follow [DISC capabilities](../../../../docs/protocol/disc-capabilities.md),
[remote control](../../../../docs/protocol/remote-control.md) and [track completion](../../../../docs/protocol/track-end.md).

## Shared Controller API

Implemented after the request journal on 2026-09-18. The common
[Controller API](../../../../controller/docs/api.md) now owns the persistent receiver/session,
...
The implemented [request journal](../guides/history.md) assigns one request ID
...
[test-selection policy](../../../../docs/development/ci.md#test-selection-policy); emulator results do not
...
[guide](../../README.md). It preserves literal album-scoped
...
- A [command table](../guides/commands.md) describing actual CLI/text commands,
...
see [playback validation](../architecture/playback.md).
...
listening-history collection and IPC remain separate work. See the [session contract](../architecture/playback.md#m2c-persistent-device-session)
and [console command table](../guides/commands.md#interactive-console).
...
See the [journal contract](../guides/history.md). This does not implement listening
...
[the API boundary](../../../../controller/docs/api.md). Next: physical session/playback acceptance
...
[interactive command reference](../guides/commands.md#interactive-console).
...
safe template parameters. See the [response contract](../reference/responses.md) and
[locale contribution guide](../reference/locales.md). Console `/response` and `/locales`
...
See [ASSISTANT_ARCHITECTURE.md](../architecture/pipeline.md) for the current contract.
...
or SDK exception bodies. See [commands](../guides/commands.md#timing-and-live-debug-traces).
...
confirmed result. See [setup, evidence and limitations](../guides/voice.md).
...
See the [comparison evidence and interpretation limits](../guides/voice.md#recorded-acceptance).
...
See [commands](../guides/commands.md) and [evaluation](../guides/voice.md#catalog-selection-evaluation).
...
[The research plan](../evaluation/nlu-research.md) defines provider boundaries,
...
[recorded catalog comparison](../guides/voice.md#recorded-catalog-comparison).


## Read-only embedding and hybrid experiment, 2026-09-18

Completed the first comparison from the expanded NLU plan in
[`assistant/nlu/evaluation`](../../assistant/nlu/evaluation/README.md): 182
...
Implemented [versioned command references and `/explain`](../reference/command-catalog.md)
...
See [full results and limitations](../../assistant/nlu/evaluation/README.md#supervised-command-study).
...
Implemented the [annotation and training workflow](../evaluation/nlu-data.md). A
...
inputs are both visible. The [workflow](../evaluation/shadow-reports.md) explains
...
Prepared a [physical-player operator worksheet](../evaluation/physical-baseline.md) with 20
...
packet was committed. See [baseline status](../evaluation/physical-baseline.md).
