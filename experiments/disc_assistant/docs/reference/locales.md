...
[Piper voice mappings](../guides/tts.md)) and optional STT language
mapping, then add a synthetic corpus as described in
[the speech guide](../guides/voice.md#reproducible-samples-and-evaluation).
...
[`responses.py`](../../assistant/responses.py). The complete
[English catalog](../../assistant/locales/replies/en.toml) is
...
Add tests alongside
[`test_responses.py`](../../assistant/tests/test_responses.py)
and [`test_languages.py`](../../assistant/tests/test_languages.py).
...
the player UI. See the [response contract](responses.md) for persistence,
...
load a command catalog. See [catalog schema, boundaries and reproduction](command-catalog.md).

The newer [v2 annotation/training workflow](../evaluation/nlu-data.md) accepts installed
...
are unnecessary. Refer to [source contracts and limitations](../architecture/interpretation-sources.md).
