...
case "${1:-help}" in
    build)
        ...
        docker run --rm --network none -v "$PWD:/repo" snowsky-disc-browser-build \
            bash /repo/experiments/browser/build-kernel.sh > work/browser-disc/kernel-build.log 2>&1 || {
                tail -60 work/browser-disc/kernel-build.log; exit 1;
            }
        docker run --rm --network none -v "$PWD:/repo" snowsky-disc-browser-build \
            python3 -B -m experiments.browser.bundle
        printf '\nStart the static server: bash experiments/browser/run.sh serve\n'
        ;;
    refresh-ui)
        python3 -m experiments.browser.bundle --ui-only
        ;;
    serve)
    ...
    *)
        printf '%s\n' 'Usage:' \
            '  bash experiments/browser/run.sh build /path/to/main_os/ota_v257' \
            '  bash experiments/browser/run.sh serve [port, default 8091]' \
            '  bash experiments/browser/run.sh refresh-ui  # stop VM first, then reload'
        ;;
esac
