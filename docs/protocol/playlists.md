...
Custom playlist creation/editing uses the [stock HTTP API](http-api.md#http-catalog-and-custom-playlists).
...
see the [EOF lifecycle](track-end.md), not a timeout-based state inference.
...
Validation is tracked in [PROTOCOL_RESEARCH.md](../../research/docs/status.md). Emulator
evidence is not a physical FiiO Control capture. Natural five-mode EOF behavior
has separate [short-track acceptance](track-end.md).
