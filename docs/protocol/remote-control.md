...
persisted `PLAY_MODE`. V2.57 also has [natural EOF acceptance](track-end.md) for
...
- Natural final stop sends zero `a103`, then state-only `a202` with state 2.
  ...
  stopped-state detector; see [EOF lifecycle and client implications](track-end.md).
...
a confirmed cause. See [app evidence](../../research/docs/reports/fiio-control-app.md#ios-paused-seek-and-play-order-capture-2026-09-15).
...
[capture details](../../research/docs/reports/fiio-control-app.md#ios-playback-and-favorites-capture-2026-09-15)
...
See [capture details](../../research/docs/reports/fiio-control-app.md#ios-current-queue-capture-2026-09-16)
...
[HTTP_API.md](http-api.md), with [remote settings/PEQ](remote-settings.md) in a
...
Follow-up file/playlist/scanner results are in [HTTP_API.md](http-api.md);
settings/PEQ are in [REMOTE_SETTINGS.md](remote-settings.md).
...
| Work mode | USB DAC/local/AirPlay control transitions and persisted enums tested; actual hardware audio remains separate. See [modes](remote-modes-themes.md). |
...
| Settings | Network indexing, gain, SPDIF, filter, DRE, channel balance and five Bluetooth source-codec preferences tested in emulator. Dedicated V2.57 [library reset](library-reset.md) is tested separately; negotiated codec and physical audio effects remain unvalidated. |
| Lock screen / cover | Physical V2.57 current-cover JPEG; emulator checks general PNG upload, five stock themes and custom lock-screen PNG/metadata. See [theme quirks](remote-modes-themes.md#lock-screen-http). |
