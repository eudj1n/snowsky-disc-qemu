...
the opt-in host LAN bridge are in [DISCOVERY.md](../../controller/docs/discovery.md).
...
- **No sshd in the stock rootfs** — avahi advertises `_ssh._tcp`, but nothing listens (port 22
  ...
  [DISKOS.md](../../research/docs/reports/diskos.md)) or the board's **UART** test pads (inittab gives a passwordless
  `/bin/sh`).
...
- FiiO Control exposes Wi-Fi file import and folder creation for DISC in user-provided
  ...
  media; see [HTTP_API.md](http-api.md) for the exact request contract and evidence.
