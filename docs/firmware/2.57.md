...
Status: **clean baseline validated locally; upstream-feature acceptance still pending**.
...
or validation of all new features. Double-click/reassignment combinations, new features
below and V2.57's internal HTTP-route/memory-probe addresses need separate work.
...
1. Run both profiles through hosted CI on the exact commit; retain V2.40 regression coverage.
2. Re-locate V2.57 runtime probes and the active HTTP routing by strings/xrefs as needed.
   Old address-based tools now refuse this build; do not guess offsets.
