...
#
# Usage:  emulator/scripts/30_tap.sh <x> <y>
#   e.g.  emulator/scripts/30_tap.sh 180 315   # tap the bottom-center button on the language screen
#
...
python3 -m emulator.runtime.inject press "$RX" "$RY" "$EV"
sleep 1                                   # let LVGL sample the pressed state
python3 -m emulator.runtime.inject release x x "$EV"
sleep 2                                    # let the UI react + redraw

mkdir -p "$SHOTS"
cp "$ROOTFS/dev/fb0" "$WORK/fb0.snap"
python3 -m emulator.runtime.fb2png "$WORK/fb0.snap" "$SHOTS" tap
