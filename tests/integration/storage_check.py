...
def run():
    ...
    command = ['bash', '-lc', 'source /repo/emulator/scripts/lib.sh; guest_run 10 /sbin/blkid']
    print('Cold blkid:', subprocess.check_output(command, text=True).strip(), flush=True)
    subprocess.run(['bash', '-lc', 'source /repo/emulator/scripts/lib.sh; sd_mount'], check=True)
