...
compose() { docker compose --env-file /dev/null -p "$PREP_ID" -f compose.yaml -f ci/compose.yml "$@"; }
cleanup() {
    compose exec -T emu bash /repo/ci/cleanup.sh || true
    compose down --volumes --timeout 5
}
trap cleanup EXIT
compose up -d --no-build emu
...
compose exec -T emu python3 -B -m firmware.profile validate /work/rootfs
# 10_setup_env's priming guest has already stopped. Omit mounted kernel views.
compose exec -T emu tar --one-file-system --exclude='./proc/*' --exclude='./dev/mqueue/*' \
