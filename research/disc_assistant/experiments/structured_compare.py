...
from research.disc_assistant.assistant.interpretation_sources import LiteralSource, SlotSource, collect
from research.disc_assistant.assistant.structured_source import StructuredSource, PROMPT, SCHEMA
...
async def run(args):
    ...
    original=json.loads((Path(__file__).parent/'nlu/slot_acceptance.json').read_text())
    extra=json.loads((Path(__file__).parent/'nlu/structured_cases.json').read_text())
