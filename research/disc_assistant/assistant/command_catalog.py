...
DIRECTORY = Path(__file__).parent / 'locales' / 'commands'
...
def source(locale):
    ...
    seen, texts = set(), set()
    if not isinstance(examples, list) or not 1 <= len(examples) <= 2000:
        raise ValueError('command catalog needs 1..2000 examples')
    for row in examples:
...
def validate_classifier(model):
    from research.disc_assistant.assistant.command_features import VERSION
