...
from research.disc_assistant.assistant.command_catalog import digest, source as catalog_source, validate_classifier
from research.disc_assistant.assistant.command_features import classify
from research.disc_assistant.assistant.interpreter import InterpretationContext, validate_intent
from research.disc_assistant.assistant.intents import parse, Intent, ControlIntent, LanguageIntent
...
def validate(value, provider, text):
    if (type(value) is not Evidence or value.source != provider.name or value.version != provider.version
            ...
            or value.label not in ('play', 'language', 'pause', 'resume', 'stop', 'next', 'previous', 'reject')
