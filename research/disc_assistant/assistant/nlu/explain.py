...
from research.disc_assistant.assistant.nlu.command_features import classify
from research.disc_assistant.assistant.nlu.intents import parse, language_target, Intent, ControlIntent, LanguageIntent, VolumeIntent
from research.disc_assistant.assistant.nlu.interpreter import validate_intent
from research.disc_assistant.assistant.nlu.languages import load_languages, normalized


def label_of(intent):
    return (intent.action if type(intent) in (ControlIntent, VolumeIntent) else 'play' if type(intent) in (Intent, AlbumIntent)
...
def preview(config, text, trace=None):
    ...
    try:
        ...
        import asyncio
        from research.disc_assistant.assistant.nlu.interpretation_sources import explain_sources
