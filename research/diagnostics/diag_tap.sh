...
cp "$ROOTFS/dev/fb0" "$WORK/d0.snap"; python3 -m emulator.runtime.fb2png "$WORK/d0.snap" "$SHOTS" d0

read RX RY < <(rot 180 315)               # Confirm button: displayed (180,315) -> raw
log "injecting tap at raw($RX,$RY)"
python3 -m emulator.runtime.inject press "$RX" "$RY" "$ROOTFS/dev/input/event1"; sleep 1
python3 -m emulator.runtime.inject release x x "$ROOTFS/dev/input/event1"; sleep 2
