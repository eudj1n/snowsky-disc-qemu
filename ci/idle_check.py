...
from remote_control import ROOT, call, snapshot
from queue_check import connection, http_client
from queue_reads_check import prepare
...
from guest_check import capture
...
def usb_reads(connected):
    for path, size, expected in (
        ...
        result = subprocess.run(['bash', '-c',
                                 'source /repo/scripts/lib.sh; guest_run 5 /bin/busybox dd "if=$1" "bs=$2" count=1',
...
async def usb_power_check():
    ...
    device = Device(ROOT)
    controls = ViewerControls(device)
...
async def main(phase='all'):
    ...
    if phase in ('all', 'quiet'):
        ...
        if phase == 'quiet':
            ...
            return
    for transport in ('tcp', 'ws'):
        subprocess.run(['python3', '/repo/tools/keys.py', 'stop'], check=True)
        configure(300)
        subprocess.run(['bash', '/repo/scripts/20_boot.sh'], check=True)
        subprocess.run(['bash', '/repo/ci/confinement.sh'], check=True)
