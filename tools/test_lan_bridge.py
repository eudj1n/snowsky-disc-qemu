...
from lan_bridge import Proxy, http_ready, sender, announce
from fiio_discovery import PAYLOAD, GROUP, PORT
...
class LanBridgeTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_failed_upstream_releases_control_slot(self):
        ...
        with patch('lan_bridge.asyncio.open_connection', AsyncMock(side_effect=OSError('offline'))):
    ...
    async def test_connection_during_readiness_suppresses_announcement(self):
        ...
        with patch('lan_bridge.http_ready', ready), \
             patch('lan_bridge.asyncio.get_running_loop', return_value=loop), \
             patch('lan_bridge.asyncio.sleep', AsyncMock(side_effect=asyncio.CancelledError)):
    ...
    async def test_announcement_gates_and_payload(self):
        for count, ready, expected in ((0, True, True), (1, True, False), (0, False, False)):
            ...
            sock = Mock()
            loop = Mock(sock_sendto=AsyncMock())
            with patch('lan_bridge.http_ready', AsyncMock(return_value=ready)), \
                 patch('lan_bridge.asyncio.get_running_loop', return_value=loop), \
                 patch('lan_bridge.asyncio.sleep', AsyncMock(side_effect=asyncio.CancelledError)):
                with self.assertRaises(asyncio.CancelledError):
    ...
    def test_sender_uses_selected_interface_and_ttl_one(self):
        with patch('lan_bridge.socket.socket') as factory:
