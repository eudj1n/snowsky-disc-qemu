...
class OTANotificationTests(unittest.TestCase):
    def test_no_api_calls_when_no_new_release(self):
        with patch('notify_ota.api') as api:
    ...
    def test_create_with_manual_process_and_stable_marker(self):
        with patch('notify_ota.api', side_effect=[[[]], {'number': 5}]) as api:
    ...
    def test_open_and_closed_issues_on_later_pages_prevent_duplicates(self):
        for state in ('open', 'closed'):
            ...
            with self.subTest(state=state), patch('notify_ota.api', return_value=pages) as api:
    ...
    def test_recovery_only_release_and_pr_marker(self):
        ...
        with patch('notify_ota.api', side_effect=[pages, {'number': 6}]) as api:
    ...
    def test_failed_listing_never_creates_issue(self):
        with patch('notify_ota.api', side_effect=RuntimeError('API error')) as api, \
    ...
    def test_api_uses_structured_stdin_and_fixed_github_host(self):
        ...
        with patch('notify_ota.subprocess.run', return_value=result) as run:
    ...
    def test_cli_error_is_sanitized(self):
        ...
        stderr = io.StringIO()
        with patch.dict(os.environ, env), patch('notify_ota.ensure_issue', side_effect=RuntimeError('secret')), \
