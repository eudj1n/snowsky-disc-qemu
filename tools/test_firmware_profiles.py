...
class ProfileTests(unittest.TestCase):
    ...
    def test_invalid_chunks_never_reach_unsquashfs(self):
        with patch('firmware_extract.subprocess.run') as run:
            with self.assertRaises(ValueError):
    ...
    def test_bad_plaintext_never_reaches_unsquashfs(self):
        ...
        with patch('firmware_extract.plaintext_digest', return_value={'sha256': 'bad', 'size': 0}), \
             patch('firmware_extract.subprocess.run') as run:
            with self.assertRaises(ValueError):
