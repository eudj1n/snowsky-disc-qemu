# Firmware V2.57 intake and emulator compatibility

Status: **active V2.57 development**, with an existing immutable
[pre-release snapshot](https://github.com/eudj1n/snowsky-disc-qemu/releases/tag/v2.57)
retained from 2026-09-13 and reclassified on 2026-09-16. Its notes preserve exact-commit
hosted validation; the eventual stable release will use a new name such as `v2.57-r1`.
V2.40 is historical; its local profile remains until cleanup. Existing volumes and
both immutable tags are unchanged.

Release scope agreed on 2026-09-13: validate the behavior implemented or affected by
the emulator, not the entire FiiO feature list. The vendor notes below are reference
information. Their unverified status does not block an emulator release and does not
become a claim that those features were tested.
...
- [Machine-readable inventory](../../inventory/v2.57.json), generated using
  `firmware/tools/firmware_inventory.py` with the pinned Docker image and both local inputs.
...
- Signature authentication was **not** performed. No explanation for the smaller rootfs
  is claimed before content comparison. The public
  [FiiO release page](https://forum.fiio.com/note/showNoteContent.do?id=202601311712087234434)
  was subsequently confirmed and recorded in the runtime profile.

## Static comparison and guarded bring-up — 2026-09-11

Exact hashes are in [the runtime profile](../../v2.57.json), with V2.40
retained separately. Both rootfs trees were extracted into network-disabled research
containers/volumes; the interactive `snowsky-disc-work` volume was not used.
...
![V2.57 clean main screen](../../../docs/images/v2.57-main.png)
![V2.57 stock scanner indexed generated media](../../../docs/images/v2.57-library-scan.png)

Reproduce using [the version-selected integration command](../../../docs/development/ci.md). No new Docker
...
or validation of all new features. The full button-assignment matrix was not exercised;
targeted checks cover the emulator's input paths. Internal diagnostics have since been
ported; see [DIAGNOSTICS.md](../../../research/docs/diagnostics.md).

## Vendor changes — reference only

Source: release notes supplied by the repository owner on 2026-09-11 and subsequently
confirmed on the FiiO release page above. These are **upstream-reported changes**,
not verified emulator features or required release tests. Keep IDs for future targeted
investigations. The possible checks below are optional unless an emulator dependency
or regression makes one relevant.

| ID | Reported change | Possible targeted investigation | Independent verification |
|---|---|---|---|
...
## Emulator compatibility evidence and release work
...
This establishes Cyrillic filesystem paths; it does not validate the broader
FW257-03 multilingual metadata/lyrics/BT claims.
...
See [DIAGNOSTICS.md](../../../research/docs/diagnostics.md) for addresses, derivation, commands and checks.
These local working-tree results do not replace the hosted release gates below.

### Release checklist

1. Run firmware-free checks and V2.57 hosted integration on each release candidate.
   Under the 2026-09-16 single-active-firmware policy, V2.40 is historical;
   it is no longer selectable in hosted CI. Its local profile remains until cleanup.
2. Runtime key/network probes and active HTTP routing are now ported by strings/xrefs
   ...
   still requires separate porting when needed. See [diagnostic evidence](../../../research/docs/diagnostics.md).
3. Cover emulator input/gesture paths, display/backlight, SD insertion/removal and
   Cyrillic paths, network adapters, PCM/browser audio and guest power confinement.
   Add targeted cases for regressions or changed emulator interfaces, not every vendor feature.
4. Keep limitations and vendor claims explicit. Full BT/USB/MCU behavior and the
   complete stock UI/codec feature matrix are not release claims.
5. Publish an unused V2.57 release revision only after firmware-free CI and V2.57 integration pass on
   the exact release commit. Record the run links in the release notes so adding
   evidence after CI does not change the commit being released.

### Release candidate preparation — 2026-09-13

The runtime fixes through `5a73883` cover SD hotplug/automatic rescans, viewer
peripherals and brightness, startup readiness, idle key-reader CPU usage and live
browser audio joining the current capture tail. The owner confirmed audible browser
playback. [Firmware-free CI on that commit](https://github.com/eudj1n/snowsky-disc-qemu/actions/runs/34708229045)
passed; the earlier hosted firmware evidence above predates these fixes. A new pair
of hosted firmware runs is required for the final candidate; do not treat those
historical links as final release validation.
