...
from firmware.tools import check_ota
...
class OTACheckTests(unittest.TestCase):
    ...
    def test_network_failure_retries_then_fails(self):
        ...
        with patch('firmware.tools.check_ota.urllib.request.build_opener', return_value=opener), \
             patch('firmware.tools.check_ota.time.sleep') as sleep, self.assertRaises(urllib.error.URLError):
    ...
    def test_cli_suppresses_error_details(self):
        stderr = io.StringIO()
        with patch('sys.argv', ['firmware.tools.check_ota']), \
             patch('firmware.tools.check_ota.fetch_catalog', side_effect=ValueError('https://private.invalid/secret')), \
